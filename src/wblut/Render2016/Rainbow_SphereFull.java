...
public class Rainbow_SphereFull extends Slide {
	LightRay startray;
	...
	public Rainbow_SphereFull(final RTO home, final String title) {
		super(home, title);
		drawhud = true;
	}
	...
	void traceRays(final double index, final int c, final boolean ignoreI, final double baseI,
		...
		for (int i = 0; i < numrays; i++) {
			startray = new LightRay(
					gf.createRayWithDirection(2000, -199.0 + (i * (398.0 / (numrays - 1))), 0, -1, 0, 0), baseI, 0);
			rays = new ArrayList<LightRay>();
			rays.add(startray);
			events = new ArrayList<RR>();
			do {
