...
public class Rainbow_BasicPrism extends Slide {
	LightRay startray;
	...
	public Rainbow_BasicPrism(final RTO home, final String title) {
		super(home, title);
		drawhud = true;
	}
	...
	@Override
	void backgroundDraw() {
		home.background(PApplet.max(0, 20 - (0.4f * slideCounter)));
	...
	void traceRays(final double index, final int c, final boolean ignoreI, final double baseI,
		...
		for (int i = 0; i < numrays; i++) {
			startray = new LightRay(gf.createRayWithDirection(2000,
					-100 + (50 * Math.sin(home.frameCount * 0.01f)) + (0.5 * i), 0, -1, 0, 0), baseI, 0);
			rays = new ArrayList<LightRay>();
			rays.add(startray);
			events = new ArrayList<RR>();
			do {
