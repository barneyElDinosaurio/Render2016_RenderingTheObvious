...
public class Rainbow_SyntheticRainbowDynamic3D extends Slide {
	...
	public Rainbow_SyntheticRainbowDynamic3D(final RTO home, final String title) {
		super(home, title);
		drawhud = true;
	}
	...
	void normalDraw() {
		...
		if ((psunangle != sunangle) || (slideCounter < 67)) {
			home.background(0);

		}
		home.cam.beginHUD();
		home.translate(PApplet.min(400, slideCounter * 6), 0);
		home.rotateY(PApplet.min(slideCounter * 0.03f, PConstants.PI / 3));
		...
		for (int i = 0; i < numdrops; i++) {
			drop.set(
					home.random(PApplet.max((-6 * slideCounter) - 200, -home.width / 4) - 10,
							PApplet.max((-6 * slideCounter) - 200, -home.width / 4) + 10),
					home.random(-800, 400), home.random(-1500, 1500));
