...
public class Rainbow_SyntheticRainbowDynamicCanyon3D extends Slide {
	...
	double sunangle;
	double psunangle;
	HE_Mesh ground1, ground2, grid1, grid2, grid3, grid4;

	public Rainbow_SyntheticRainbowDynamicCanyon3D(final RTO home, final String title) {
		super(home, title);
		drawhud = true;
	}
	...
	public void setup() {
		...
		ground1 = new HE_Mesh(new HEC_Grid(6, 30, 600, 3000).setCenter(450, 400, 0).setZAxis(0, 1, 0));
		ground2 = new HE_Mesh(new HEC_Grid(14, 30, 1400, 3000).setCenter(-550, 800, 0).setZAxis(0, 1, 0));
		grid3 = new HE_Mesh(new HEC_Grid(30, 4, 3000, 400).setCenter(150, 600, 0).setZAxis(1, 0, 0));
		grid1 = new HE_Mesh(new HEC_Grid(20, 15, 2000, 1200).setCenter(-250, -200, -1500).setZAxis(0, 0, 1));
		grid4 = new HE_Mesh(new HEC_Grid(14, 4, 1400, 400).setCenter(-550, 600, -1500).setZAxis(0, 0, 1));
	}
	...
	void normalDraw() {
		...
		if ((psunangle != sunangle)) {
			...
			home.stroke(255, 200);
			render.drawEdges(ground1);
			render.drawEdges(ground2);
			render.drawEdges(grid1);
			render.drawEdges(grid3);
			render.drawEdges(grid4);
			home.pushMatrix();
		...
		for (int i = 0; i < numdrops; i++) {
			drop.set(home.random((-home.width / 4) - 10, (-home.width / 4) + 10), home.random(-800, 800),
	...
	@Override
	public void hudDraw() {
		if ((psunangle != sunangle) || (slideCounter < ((100 * PConstants.PI) / 3))) {
			home.noLights();
