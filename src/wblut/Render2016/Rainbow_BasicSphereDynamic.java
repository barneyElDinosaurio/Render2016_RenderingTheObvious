...
public class Rainbow_BasicSphereDynamic extends Slide {
	LightRay startray;
	...
	public Rainbow_BasicSphereDynamic(final RTO home, final String title) {
		super(home, title);
		drawhud = true;
	}
	...
	void glowDraw() {
		home.strokeWeight(2);
		/*
		 * for(int i=450;i<=650;i+=100){ traceRays(i, ignoreI, baseI); }
		 */
	...
	void traceRays(final double index, final int c, final boolean ignoreI, final double baseI,
		...
		for (int i = 0; i < numrays; i++) {
			startray = new LightRay(
					gf.createRayWithDirection(2000, -239.9 + ((home.frameCount + 90 + (0.1 * i)) % 480), 0, -1, 0, 0),
					baseI, 0);
