...
public class Rainbow_SphereWithReflectionSweetspot extends Slide {
	LightRay startray;
	...
	public Rainbow_SphereWithReflectionSweetspot(final RTO home, final String title) {
		super(home, title);
		drawhud = true;
	}
	...
	void traceRays(final double index, final int c, final boolean ignoreI, final double baseI,
		...
		for (int i = 0; i < numrays; i++) {
			startray = new LightRay(gf.createRayWithDirection(2000, -200 + (0.025 * i), 0, -1, 0, 0), baseI, 0);
			rays = new ArrayList<LightRay>();
			rays.add(startray);
			events = new ArrayList<RR>();
			do {
